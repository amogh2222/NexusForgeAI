...
r = requests.post(f"{API_URL}/auth/register", json={"email": email, "password": password, "name": "Test User"})
if r.status_code != 200: fail(f"Register failed: {r.text}")
...
if r.status_code != 200: fail(f"Project creation failed: {r.text}")
...
if r.status_code != 200: fail(f"Repo add failed: {r.text}")
repo_id = r.json()["id"]
